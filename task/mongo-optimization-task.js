...
async function task_3_1(db) {
    ...
    const result = await db.collection('opportunities').aggregate([
        {$match: {
          ...
          }}, {$project: 
          {
          ...
          }}, {$unwind: '$contacts.questions'}, {$project: {
            _id: 1,
            'contacts.id': 1,
          ...
          }}, {$unwind: '$contacts.questions.answers'}, {$match: {
            'contacts.questions.answers.primary_answer_value': {
              $lt: 9000
            }
          }}, {$unwind: '$contacts.questions.answers.loopInstances'}, {$project: {
            _id: 1,
            'contacts.id': 1,
            'contacts.questions.criteria_value': 1,
            criteria_value: {
            ...
            'contacts.questions.answers.loopInstances.is_selected': 1,
              ...
              'criteria_label': { $arrayElemAt: ['$criteria.label', 0] },
            'criteria_versions_definition': { $arrayElemAt: 
                                                [ { $arrayElemAt: ['$criteria.versions.definition', 0] }, 0] 
                                            },
            'criteria_definition': { $arrayElemAt: ['$criteria.definition', 0] },
          }}, {$group: {
